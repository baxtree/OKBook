    function showgraph(im, base, graph_id){
		var analysis_button = document.getElementById("graph");
		...
		var url = "http://localhost:10081/IMAnnotator/org/openk/annotator/servlet/TripleHarvesterServlet";
		var para = "rdfacontent=" + escape(im_published_content) + "&baseuri=" + escape(base_uri);
			...
			if(req.readyState == 4){
				if(req.status == 200){
					...
					if(req.responseText === "none"){
						graph.innerHTML = "<span style='color:red'>Sorry. No annotation found for this interaction model</span>";
					}
					else{
						//var triples = new Array(req.responseText);
						//var triples = eval(req.responseText);
						// Load the Visualization API and the piechart package.
						google.load('visualization', '1', {'packages':['table']}); 		
                        alert("google visualization api loaded!");						
	...
	function drawChart(){
		...
		data.addRows(triples.length);
		for(var i = 0; i < triples.length; i++){
			data.setCell(i, 0, triples[i][0]);
			data.setCell(i, 1, triples[i][1]);
			data.setCell(i, 2, triples[i][2]);
		}
		*/
